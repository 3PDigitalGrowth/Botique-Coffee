...
6. Tested 2 September 2026: two TEST enquiries posted through the production API exactly as the lander sends them; both delivered by email, the second landed in the leads sheet with gclid, keyword, match type, location ID and intent in the comments column ("Delivered"). The first was lost to a cold-start timeout on the sheet append, since fixed (10s budget plus a retry). Thank-you page and landers carry the AW-18421695400 tag; the conversion will show in Google Ads once a real ad click reaches the thank-you page. Ads are in policy review (REVIEW_IN_PROGRESS on all four RSAs at build time).
